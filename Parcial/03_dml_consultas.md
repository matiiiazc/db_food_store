...
| 6 | **Subconsulta correlacionada** | Producto vs. promedio de su categoría (outer acotado a top 5 por precio para que el ORDER BY sea viable en vivo) |
...
- Consultas 3, 5, 6, 7, 10 verificadas por ejecución directa en PostgreSQL 18 (resultados correctos arriba). La consulta 6 se ajustó para la entrega: sin acotar, el `ORDER BY` sobre la columna calculada obliga a evaluar la subconsulta por las 50.000 filas (plan de costo ~22M, no viabilizable en defensa); con el outer en top 5 ejecuta en ~53 ms.
